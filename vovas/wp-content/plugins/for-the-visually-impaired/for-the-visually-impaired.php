...
//if (htmlspecialchars(get_option('wfvi_plugin_version')!=WFVI_PLUGIN_VERSION_NUM) {	
...
//    check_admin_referer();
    ...
    $data_js_in_header = 'ftvi_opt_js_in_header';	
    ...
    $opt_val_txt_size = htmlspecialchars(get_option( $opt_txt_size ));
    $opt_val_sxema = htmlspecialchars(get_option( $opt_sxema ));
    $opt_val_image = htmlspecialchars(get_option( $opt_image ));
    $opt_val_font = htmlspecialchars(get_option( $opt_font ));
    $opt_val_kerning = htmlspecialchars(get_option( $opt_kerning ));
    $opt_val_line = htmlspecialchars(get_option( $opt_line ));
    $opt_val_razmer_images = htmlspecialchars(get_option( $opt_razmer_images ));
    $opt_val_allbackgrnd = htmlspecialchars(get_option( $opt_allbackgrnd ));
    $opt_val_reload_page = htmlspecialchars(get_option( $opt_reload_page ));
	$opt_val_auto_p_on = htmlspecialchars(get_option( $opt_auto_p_on ));
	$opt_val_scrool_panel = htmlspecialchars(get_option( $opt_scrool_panel ,"1"));
	$opt_val_js_in_header = htmlspecialchars(get_option( $opt_js_in_header ,"0"));
    $opt_val_bcolor = htmlspecialchars(get_option( $opt_bcolor ));
    $opt_val_btxtcolor = htmlspecialchars(get_option( $opt_btxtcolor ));
    $opt_val_bimage = htmlspecialchars(get_option( $opt_bimage ));
    $opt_val_bimage2 = htmlspecialchars(get_option( $opt_bimage2 ));
    $opt_val_btxt = htmlspecialchars(get_option( $opt_btxt ));
    $opt_val_btxt2 = htmlspecialchars(get_option( $opt_btxt2 ));
	if (!empty($_POST[ $hidden_field_name ])){$optfn=htmlspecialchars($_POST[ $hidden_field_name ]);}else{$optfn="";}
    if ( $optfn== 'Y' ) {
		if (( $opt_val_txt_size ) != $_POST[ $data_txt_size ] ) {$opt_val_txt_size=htmlspecialchars($_POST[ $data_txt_size ]); update_option( $opt_txt_size, $opt_val_txt_size);}
		if (( $opt_val_sxema ) != $_POST[ $data_sxema ] ) {$opt_val_sxema=htmlspecialchars($_POST[ $data_sxema ]);update_option( $opt_sxema, $opt_val_sxema );}
		if (( $opt_val_image ) != $_POST[ $data_image ] ) {$opt_val_image=htmlspecialchars($_POST[ $data_image ]);update_option( $opt_image, $opt_val_image);}
		if (( $opt_val_font ) != $_POST[ $data_font ] ) {$opt_val_font=htmlspecialchars($_POST[ $data_font ]);update_option( $opt_font, $opt_val_font );}
		if (( $opt_val_kerning ) != $_POST[ $data_kerning ] ) {$opt_val_kerning=htmlspecialchars($_POST[ $data_kerning ]);update_option( $opt_kerning, $opt_val_kerning );}
		if (( $opt_val_line ) != $_POST[ $data_line ] ) {$opt_val_line=htmlspecialchars($_POST[ $data_line ]);update_option( $opt_line, $opt_val_line );}
		if (( $opt_val_razmer_images ) != $_POST[ $data_razmer_images ] ) {$opt_val_razmer_images=htmlspecialchars($_POST[ $data_razmer_images ]) ;update_option( $opt_razmer_images,$opt_val_razmer_images);}
		if (( $opt_val_allbackgrnd ) != $_POST[ $data_allbackgrnd ] ) {$opt_val_allbackgrnd=htmlspecialchars($_POST[ $data_allbackgrnd ]) ;update_option( $opt_allbackgrnd,$opt_val_allbackgrnd);}
		if (( $opt_val_reload_page ) != $_POST[ $data_reload_page ] ) {$opt_val_reload_page=htmlspecialchars($_POST[ $data_reload_page ]);update_option( $opt_reload_page,$opt_val_reload_page);}
		if (( $opt_val_auto_p_on ) != $_POST[ $data_auto_p_on ] ) {$opt_val_auto_p_on=htmlspecialchars($_POST[ $data_auto_p_on ]);update_option( $opt_auto_p_on,$opt_val_auto_p_on);}				
		if (( $opt_val_scrool_panel ) != $_POST[ $data_scrool_panel ] ) {$opt_val_scrool_panel=htmlspecialchars($_POST[ $data_scrool_panel ]);update_option( $opt_scrool_panel,$opt_val_scrool_panel);}		
		if (( $opt_val_js_in_header ) != $_POST[ $data_js_in_header ] ) {$opt_val_js_in_header=htmlspecialchars($_POST[ $data_js_in_header ]);update_option( $opt_js_in_header,$opt_val_js_in_header);}		
		if (( $opt_val_bcolor ) != $_POST[ $data_bcolor ] ) {$opt_val_bcolor=htmlspecialchars($_POST[ $data_bcolor ]);update_option( $opt_bcolor, $opt_val_bcolor);}
		if (( $opt_val_btxtcolor ) != $_POST[ $data_btxtcolor ] ) {$opt_val_btxtcolor=htmlspecialchars($_POST[ $data_btxtcolor ]);update_option( $opt_btxtcolor, $opt_val_btxtcolor);}
		if (( $opt_val_bimage ) != $_POST[ $data_bimage ] ) {$opt_val_bimage=htmlspecialchars($_POST[ $data_bimage ]) ;update_option( $opt_bimage,$opt_val_bimage );}
		if (( $opt_val_bimage2 ) != $_POST[ $data_bimage2 ] ) {$opt_val_bimage2=htmlspecialchars($_POST[ $data_bimage2 ]) ;update_option( $opt_bimage2,$opt_val_bimage2 );}
		if (( $opt_val_btxt ) != $_POST[ $data_btxt ] ) {$opt_val_btxt=htmlspecialchars($_POST[ $data_btxt ]) ;update_option( $opt_btxt, $opt_val_btxt);}
		if (( $opt_val_btxt2 ) != $_POST[ $data_btxt2 ] ) {$opt_val_btxt2=htmlspecialchars($_POST[ $data_btxt2 ]) ;update_option( $opt_btxt2, $opt_val_btxt2);}
?>
...
function vi_cookie(){
	wp_register_script('vi_cookie',plugin_dir_url( __FILE__ ).'js/js.cookie.js', array('jquery'),'2.1.0', htmlspecialchars(get_option( 'ftvi_option_js_in_header' )));
...
function add_vi_script(){
	...
	wp_register_script('add_vi_script',plugin_dir_url( __FILE__ ).'js/js.for.the.visually.impaired.js', array('jquery'),'0.53', htmlspecialchars(get_option( 'ftvi_option_js_in_header' )));
	wp_enqueue_script('add_vi_script');
	 $opt_val_txt_size = htmlspecialchars(get_option( 'ftvi_option_txt_size' ));
    $opt_val_sxema = htmlspecialchars(get_option( 'ftvi_option_sxema' ));
    $opt_val_image = htmlspecialchars(get_option( 'ftvi_option_image' ));
    $opt_val_font = htmlspecialchars(get_option( 'ftvi_option_font' ));
    $opt_val_kerning = htmlspecialchars(get_option( 'ftvi_option_kerning' ));
    $opt_val_line = htmlspecialchars(get_option( 'ftvi_option_line' ));
    $opt_val_allbackgrnd = htmlspecialchars(get_option( 'ftvi_option_allbackgrnd' ));
    $opt_val_reload_page = htmlspecialchars(get_option( 'ftvi_option_reload_page' ));
	$opt_val_auto_p_on = htmlspecialchars(get_option( 'ftvi_option_auto_p_on' ));
	$opt_val_scrool_panel = htmlspecialchars(get_option( 'ftvi_option_scrool_panel' ));
    $opt_val_btxt = htmlspecialchars(get_option( 'ftvi_option_btxt' ));
    $opt_val_btxt2 = htmlspecialchars(get_option( 'ftvi_option_btxt2' ));
    $opt_val_btxtcolor = htmlspecialchars(get_option( 'ftvi_option_btxtcolor' ));
    $opt_val_bcolor = htmlspecialchars(get_option( 'ftvi_option_bcolor' ));
	$opt_val_razmer_images = htmlspecialchars(get_option( 'ftvi_option_razmer_images' ));
...
function add_vi_styles() {
    ...
    wp_enqueue_style('vi-style');
		...
		$opt_val_bimage = htmlspecialchars(get_option( 'ftvi_option_bimage','1' ));
		
		$opt_val_bcolor = htmlspecialchars(get_option( 'ftvi_option_bcolor','#DB151C' ));
	    $opt_val_btxtcolor = htmlspecialchars(get_option( 'ftvi_option_btxtcolor','#ffffff' ));
...
'ID' => 1,
 ...
 'title' => htmlspecialchars(get_option( 'ftvi_option_btxt' )),
 'url' => '#',
 'target' => '',
 'attr_title' => '',
 'description' => '',
...
(object) array( 
 ...
 'object' => 'my-vi-object',
 'type_label' => 'FTVI-menu',
 'title' => vi_get_img_src().' '.htmlspecialchars(get_option( 'ftvi_option_btxt' )),
 'url' => '#',
 'target' => '',
 'attr_title' => '',
 'description' => '',
...
function vi_menu_filter($items, $menu, $args ) {
...
//if ( get_query_var( 'post_type' ) == $item->type ) {
//$item->title = '<a href="#">'.vi_get_img_src().' '.htmlspecialchars(get_option( 'ftvi_option_btxt' ).'</a>';
//$item->classes []= 'current-menu-item';
...
//}
//$items->title = '<a href="#">'.vi_get_img_src().' '.htmlspecialchars(get_option( 'ftvi_option_btxt' ).'</a>'; 
return $items;
...
$img="";
    ...
    $opt_val_bimage = htmlspecialchars(get_option( 'ftvi_option_bimage' ));
...
$img="";
    $opt_val_bimage2 = htmlspecialchars(get_option('ftvi_option_bimage2'));
...
function vi_activation_plugin()
    {
if (!htmlspecialchars(get_option('ftvi_option_txt_size'))) update_option( 'ftvi_option_txt_size', '16' );
if (!htmlspecialchars(get_option('ftvi_option_sxema'))) update_option( 'ftvi_option_sxema', '1' );
if (!htmlspecialchars(get_option('ftvi_option_image'))) update_option( 'ftvi_option_image', '0' );
if (!htmlspecialchars(get_option('ftvi_option_font'))) update_option( 'ftvi_option_font', '1' );
if (!htmlspecialchars(get_option('ftvi_option_kerning'))) update_option( 'ftvi_option_kerning', '0' );
if (!htmlspecialchars(get_option('ftvi_option_line'))) update_option( 'ftvi_option_line', '0');
if (!htmlspecialchars(get_option('ftvi_option_allbackgrnd'))) update_option( 'ftvi_option_allbackgrnd', '1');
if (!htmlspecialchars(get_option('ftvi_option_reload_page'))) update_option( 'ftvi_option_reload_page', '0');
if (!htmlspecialchars(get_option('ftvi_option_auto_p_on'))) update_option( 'ftvi_option_auto_p_on', '0');
if (!htmlspecialchars(get_option('ftvi_option_bcolor'))) update_option( 'ftvi_option_bcolor', '#DB151C' );
if (!htmlspecialchars(get_option('ftvi_option_btxtcolor'))) update_option( 'ftvi_option_btxtcolor', '#000000' );
if (!htmlspecialchars(get_option('ftvi_option_bimage'))) update_option( 'ftvi_option_bimage', '0');
if (!htmlspecialchars(get_option('ftvi_option_bimage2'))) update_option( 'ftvi_option_bimage2', '0' );
if (!htmlspecialchars(get_option('ftvi_option_btxt'))) update_option( 'ftvi_option_btxt', __( 'Version for visually impaired', 'for-the-visually-impaired' ));
if (!htmlspecialchars(get_option('ftvi_option_btxt2'))) update_option( 'ftvi_option_btxt2', __( 'BACK TO NORMAL MODE', 'for-the-visually-impaired' ));
	}
...
add_filter('wp_get_nav_menu_items', 'vi_menu_filter',10,3);
...
add_action('widgets_init', 'wp_vi_pluginsx');
function wp_vi_pluginsx(){
   return register_widget("wp_vi_plugin");
}
